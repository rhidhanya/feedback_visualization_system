...
import { FiEye, FiEyeOff, FiLoader, FiAlertTriangle, FiCoffee } from 'react-icons/fi';
import { CampusLensIcon } from '../components/CollegePulseLogo';
import { useAuth } from '../context/AuthContext';

const MessInchargeLogin = () => {
    const { messInchargeLogin, loading } = useAuth();
    ...
    const handleSubmit = async (e) => {
        ...
        const result = await messInchargeLogin(emailToSubmit, form.password);
        if (!result.success) { setError(result.message); return; }
        // Verify the user is mess domain head
        if (result.assignedDomain !== 'mess') {
            setError('Unauthorized. Only Mess Incharge can access this portal.');
            return;
        }
        ...
        <div className="login-page">
            <div className="login-card">
                ...
                <div style={{
                    ...
                    <FiCoffee size={14} style={{ color: 'var(--clr-primary)', flexShrink: 0 }} />
                    <span style={{ color: 'var(--clr-primary)' }}>Restricted to <strong>Mess Incharge</strong> only</span>
                </div>

                <form className="login-form" onSubmit={handleSubmit} id="mess-incharge-login-form">
                    {error && (
                    ...
                    <div className="input-group">
                        <label>Email</label>
                        <input id="mess-email-input" type="text" value={form.email}
                            onChange={e => setForm(p => ({ ...p, email: e.target.value }))}
                            placeholder="mess-manager (or mess-manager@bitsathy.in)" required autoComplete="email" />
                    </div>
                    <div className="input-group">
                        <label>Password</label>
                        <div style={{ position: 'relative' }}>
                            <input id="mess-password-input" type={showPass ? 'text' : 'password'}
                                value={form.password}
                    ...
                    <button id="mess-login-btn" type="submit" className="btn btn-primary btn-full" disabled={loading}
                        style={{ background: 'var(--clr-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                        {loading ? <><FiLoader size={15} /> Signing in...</> : <><FiCoffee size={15} /> Sign In</>}
                    </button>
                </form>
...
export default MessInchargeLogin;
