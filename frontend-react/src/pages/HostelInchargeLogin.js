...
import { FiEye, FiEyeOff, FiLoader, FiAlertTriangle, FiHome } from 'react-icons/fi';
import { CampusLensIcon } from '../components/CollegePulseLogo';
import { useAuth } from '../context/AuthContext';

const HostelInchargeLogin = () => {
    const { hostelInchargeLogin, loading } = useAuth();
    ...
    const handleSubmit = async (e) => {
        ...
        const result = await hostelInchargeLogin(emailToSubmit, form.password);
        if (!result.success) { setError(result.message); return; }
        // Verify the user is hostel domain head
        if (result.assignedDomain !== 'hostel') {
            setError('Unauthorized. Only Hostel Incharge can access this portal.');
            return;
        }
        ...
        <div className="login-page">
            <div className="login-card">
                ...
                <div style={{
                    ...
                    <FiHome size={14} style={{ color: 'var(--clr-primary)', flexShrink: 0 }} />
                    <span style={{ color: 'var(--clr-primary)' }}>Restricted to <strong>Hostel Incharge</strong> only</span>
                </div>

                <form className="login-form" onSubmit={handleSubmit} id="hostel-incharge-login-form">
                    {error && (
                    ...
                    <div className="input-group">
                        <label>Email</label>
                        <input id="hostel-email-input" type="text" value={form.email}
                            onChange={e => setForm(p => ({ ...p, email: e.target.value }))}
                            placeholder="hostel-manager (or hostel-manager@bitsathy.in)" required autoComplete="email" />
                    </div>
                    <div className="input-group">
                        <label>Password</label>
                        <div style={{ position: 'relative' }}>
                            <input id="hostel-password-input" type={showPass ? 'text' : 'password'}
                                value={form.password}
                    ...
                    <button id="hostel-login-btn" type="submit" className="btn btn-primary btn-full" disabled={loading}
                        style={{ background: 'var(--clr-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                        {loading ? <><FiLoader size={15} /> Signing in...</> : <><FiHome size={15} /> Sign In</>}
                    </button>
                </form>
...
export default HostelInchargeLogin;
