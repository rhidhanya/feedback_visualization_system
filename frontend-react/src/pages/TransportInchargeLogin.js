...
import { FiEye, FiEyeOff, FiLoader, FiAlertTriangle, FiTruck } from 'react-icons/fi';
import { CampusLensIcon } from '../components/CollegePulseLogo';
import { useAuth } from '../context/AuthContext';

const TransportInchargeLogin = () => {
    const { transportInchargeLogin, loading } = useAuth();
    ...
    const handleSubmit = async (e) => {
        ...
        const result = await transportInchargeLogin(emailToSubmit, form.password);
        if (!result.success) { setError(result.message); return; }
        // Verify the user is transport domain head
        if (result.assignedDomain !== 'transport') {
            setError('Unauthorized. Only Transport Incharge can access this portal.');
            return;
        }
        ...
        <div className="login-page">
            <div className="login-card">
                ...
                <div style={{
                    ...
                    <FiTruck size={14} style={{ color: 'var(--clr-primary)', flexShrink: 0 }} />
                    <span style={{ color: 'var(--clr-primary)' }}>Restricted to <strong>Transport Incharge</strong> only</span>
                </div>

                <form className="login-form" onSubmit={handleSubmit} id="transport-incharge-login-form">
                    {error && (
                    ...
                    <div className="input-group">
                        <label>Email</label>
                        <input id="transport-email-input" type="email" value={form.email}
                            onChange={e => setForm(p => ({ ...p, email: e.target.value }))}
                            placeholder="transport@bitsathy.in" required autoComplete="email" />
                    </div>
                    <div className="input-group">
                        <label>Password</label>
                        <div style={{ position: 'relative' }}>
                            <input id="transport-password-input" type={showPass ? 'text' : 'password'}
                                value={form.password}
                    ...
                    <button id="transport-login-btn" type="submit" className="btn btn-primary btn-full" disabled={loading}
                        style={{ background: 'var(--clr-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                        {loading ? <><FiLoader size={15} /> Signing in...</> : <><FiTruck size={15} /> Sign In</>}
                    </button>
                </form>
...
export default TransportInchargeLogin;
